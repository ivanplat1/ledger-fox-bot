-- Тестовые данные для ExpenseCatBot (KZT - Тенге)
...
-- Вставка банковских транзакций (bank_transactions) - KZT
...
-- Проверка данных
-- SELECT COUNT(*) FROM receipts WHERE currency = 'KZT';
-- SELECT COUNT(*) FROM expenses WHERE currency = 'KZT';
-- SELECT COUNT(*) FROM bank_transactions WHERE currency = 'KZT';
-- 
-- SELECT period, SUM(amount) as total FROM expenses WHERE currency = 'KZT' GROUP BY period ORDER BY period DESC;
-- SELECT category, SUM(amount) as total FROM expenses WHERE currency = 'KZT' GROUP BY category ORDER BY total DESC;
